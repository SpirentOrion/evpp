...
static std::unordered_set<FdChannel*> validObjects;
...
FdChannel::FdChannel(EventLoop* l, evpp_socket_t f, bool r, bool w)
	...
	validObjects.insert(this);
}

FdChannel::~FdChannel() {
    EVPP_DLOG_TRACE << "fd=" << fd_;
    assert(event_ == nullptr);
	validObjects.erase(this);
}
...
void FdChannel::AttachToLoop() {
    ...
    if (EventAdd(event_, nullptr) == 0) {
        EVPP_DLOG_TRACE << "fd=" << fd_ << " watching event " << EventsToString();
        attached_ = true;
    } else {
...
void FdChannel::DetachFromLoop() {
    ...
    if (EventDel(event_) == 0) {
        attached_ = false;
        EVPP_DLOG_TRACE << "fd=" << fd_ << " detach from event loop";
    } else {
...
void FdChannel::HandleEvent(evpp_socket_t sockfd, short which) {
    ...
    if ((which & kReadable) && read_fn_) {
		if (validObjects.find(this) != validObjects.end())
		{
			read_fn_();
		}
    }
